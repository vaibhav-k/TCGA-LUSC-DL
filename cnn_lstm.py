...
def train_cnn_lstm_model(X, y):
    ...
    for train, test in kfold.split(X, y):
        ...
        model.add(
            TimeDistributed(
                Conv2D(1, kernel_size=2, activation="relu", input_shape=(4024, 2, 4, 1))
            )
        )
        ...
        tensorboard = TensorBoard(
            log_dir="logs/fit/cnn-lstm/", histogram_freq=1, write_images=True
        )
    ...
    plot_model(model, to_file="cnn-lstm.png", show_shapes=True)
...
train_cnn_lstm_model(X_rgb_resized, y)
